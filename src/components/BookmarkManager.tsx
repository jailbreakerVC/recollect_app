...
const BookmarkManager: React.FC = () => {
  ...
  useEffect(() => {
    const handleExtensionMessage = (event: MessageEvent) => {
      if (event.data.source === "bookmark-manager-extension") {
        console.log(
          "📨 BookmarkManager received extension message:",
          event.data,
        );

        switch (event.data.action) {
          case "searchByKeyword":
            handleKeywordSearchRequest(
              event.data.keyword,
              event.data.requestId,
            );
            break;
          case "searchByPageContext":
            handlePageContextSearchRequest(
              event.data.context,
              event.data.requestId,
            );
            break;
  ...
  const handleKeywordSearchRequest = async (
    keyword: string,
    requestId: string,
  ) => {
    ...
    try {
      Logger.info("BookmarkManager", "Handling keyword search request", {
        keyword,
      });
      ...
      Logger.info(
        "BookmarkManager",
        `Sent ${results.length} search results for keyword: ${keyword}`,
      );
    } catch (error) {
  ...
  const handlePageContextSearchRequest = async (
    context: any,
    requestId: string,
  ) => {
    console.log("🤖 Handling page context search request:", {
      context,
      requestId,
    });
    ...
    try {
      Logger.info(
        "BookmarkManager",
        "Handling page context search request",
        context,
      );
      ...
      Logger.info(
        "BookmarkManager",
        `Sent ${results.length} contextual search results for page: ${context.title}`,
      );
    } catch (error) {
  ...
  const searchBookmarksByKeyword = async (keyword: string) => {
    ...
    console.log(
      `🔍 Keyword search for "${keyword}" found ${results.length} results`,
    );
    return results;
  };
  ...
  const searchBookmarksByPageContext = async (context: any) => {
    ...
    console.log(
      `🤖 Context search for "${title}" found ${results.length} results`,
    );
    return results;
  };
  ...
  const calculateContextSimilarity = (
    bookmark: any,
    searchTerms: string[],
    domain: string,
  ): number => {
    let score = 0;
    const bookmarkText =
      `${bookmark.title} ${bookmark.url} ${bookmark.folder || ""}`.toLowerCase();

    // Check for term matches
    const matchingTerms = searchTerms.filter((term) =>
      bookmarkText.includes(term),
    );
    ...
    const hasTechMatch = techKeywords.some(
      (tech) =>
        bookmarkText.includes(tech) &&
        searchTerms.some((term) => term.includes(tech)),
    );
    if (hasTechMatch) {
      score += 0.2;
    }
  ...
  const handleAddBookmark = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showSuccess(
        "Bookmark Added",
        `"${newBookmark.title}" has been added successfully`,
      );
  ...
  const handleRemoveBookmark = async (id: string) => {
    ...
    if (
      window.confirm(`Are you sure you want to remove "${bookmark.title}"?`)
    ) {
      const loadingToastId = showLoading(
        "Removing Bookmark",
        "Deleting bookmark...",
  ...
  //     //   "Chrome extension is not installed or not responding. Please install the extension and refresh the page.",
  //     // );
  //     // return;
  //   }
  ...
  //       showError(
  //         "Extension Error",
  //         `Failed to connect to Chrome extension: ${extTest.message}`,
  //       );
  //     }
  ...
  //       showError(
  //         "Database Error",
  //         `Failed to connect to database: ${dbTest.message}`,
  //       );
  //       return;
  //     }
  ...
  //     removeToast(loadingToastId);
  //     const message =
  //       err instanceof Error ? err.message : "Failed to sync with extension";
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      removeToast(loadingToastId);
      const message =
        err instanceof Error ? err.message : "Failed to refresh bookmarks";
  ...
  const handleDebugRefresh = async () => {
    if (user) {
      try {
        ...
        showSuccess(
          "Debug Info Updated",
          "User context debug information refreshed",
        );
      } catch (err) {
  ...
  const handleDebugSync = async () => {
    ...
    if (extensionStatus !== "available") {
      showError(
        "Debug Sync Failed",
        `Chrome extension not available (status: ${extensionStatus})`,
      );
      return;
    }
    ...
    try {
      ...
      const dbChromeIds = new Set(
        databaseBookmarks.map((b) => b.chrome_bookmark_id).filter(Boolean),
      );

      const newInExtension = extensionBookmarks.filter(
        (b) => !dbChromeIds.has(b.id),
      );
      const removedFromExtension = databaseBookmarks.filter(
        (b) => b.chrome_bookmark_id && !extIds.has(b.chrome_bookmark_id),
      );

      removeToast(loadingToastId);
      showSuccess(
        "Debug Analysis Complete",
        `Extension: ${extensionBookmarks.length} bookmarks, Database: ${databaseBookmarks.length} bookmarks, New: ${newInExtension.length}, Removed: ${removedFromExtension.length}. Check console for details.`,
      );
    } catch (err) {
      removeToast(loadingToastId);
      showError(
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <h1 className="text-2xl font-bold text-gray-900">Recollect</h1>
            ...
            <div className="flex items-start">
              ...
              <div className="ml-4 flex-1">
                ...
                <p className="text-green-800 mb-4">
                  Your Chrome extension now provides intelligent bookmark
                  search! Right-click any text to search your bookmarks, and get
                  automatic suggestions based on the pages you visit.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="bg-white bg-opacity-50 p-3 rounded-lg">
                    ...
                    <p className="text-green-700">
                      ...
                      "Search Bookmarks" to find related bookmarks instantly.
                    </p>
                  </div>
                  <div className="bg-white bg-opacity-50 p-3 rounded-lg">
                    <h4 className="font-semibold text-green-900 mb-1">
                    ...
                    <p className="text-green-700">
                      The extension automatically analyzes pages you visit and
                      suggests relevant bookmarks from your collection.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center">
              <div className="flex-1">
                ...
                <p className="text-red-600 text-sm mt-2">
                  Please check your Supabase configuration and ensure your
                  project is active.
                </p>
              </div>
              <button
