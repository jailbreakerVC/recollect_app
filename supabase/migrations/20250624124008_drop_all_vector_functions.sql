...
DROP FUNCTION IF EXISTS search_bookmarks_vector(vector(384), text, float, int);
DROP FUNCTION IF EXISTS search_bookmarks_vector(vector(384), text, int);
DROP FUNCTION IF EXISTS search_bookmarks_vector(vector(384), text);

-- Then drop the vector column and recreate it
ALTER TABLE bookmarks DROP COLUMN IF EXISTS title_embedding;
ALTER TABLE bookmarks ADD COLUMN title_embedding vector(384);
...
BEGIN
    ...
    AND 1 - (b.title_embedding <> query_embedding) > 0.1  -- Lower threshold for more results
