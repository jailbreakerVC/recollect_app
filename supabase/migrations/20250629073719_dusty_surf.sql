/*
  ...
  1. Database Changes
    - Add unique constraint to prevent duplicate bookmarks for same user with same title
    - Create function to handle upsert operations (insert or update existing)
    - Add indexes for better performance on duplicate checking

  2. Functions
    - upsert_bookmark: Insert new bookmark or update existing one
    - check_bookmark_exists: Check if bookmark already exists for user
    - merge_bookmark_data: Merge bookmark data when duplicates are found

  3. Security
    - Maintain existing RLS policies
    - Ensure data integrity across devices
*/
...
BEGIN
  ...
  FOR bookmark_item IN SELECT * FROM jsonb_array_elements(p_bookmarks)
  LOOP
    BEGIN
      ...
      results_array := results_array || jsonb_build_object(
        'id', upsert_result.id,
        'title', upsert_result.title,
        'operation', CASE WHEN upsert_result.was_updated THEN 'updated' ELSE 'inserted' END
      );
...
GRANT EXECUTE ON FUNCTION bulk_upsert_bookmarks(text, jsonb) TO anon, authenticated, service_role;
